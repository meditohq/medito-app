...
void main() {
  ...
  testWidgets(
    'another pack shows "Set as Your Path"; tapping it makes it the path',
    (tester) async {
      await pump(tester, sleep);

      expect(find.text(l10n.setAsYourPath), findsOneWidget);

      await tester.tap(find.text(l10n.setAsYourPath));
      await tester.pumpAndSettle();

      expect(prefs.getString(SharedPreferenceConstants.upNextPackId), 'sleep');
      // Button flips to the current-path state with progress.
      expect(find.text(l10n.yourPathRowSubtitle(3, 7)), findsOneWidget);
    },
  );

  testWidgets('current pack opens a sheet whose remove action falls back to '
      'the default pack', (tester) async {
    await prefs.setString(SharedPreferenceConstants.upNextPackId, 'sleep');
    await pump(tester, sleep);

    await tester.tap(find.text(l10n.yourPathRowSubtitle(3, 7)));
    await tester.pumpAndSettle();
  ...
  testWidgets('default pack as current path cannot be removed', (tester) async {
    ...
    await tester.tap(find.text(l10n.yourPathRowSubtitle(1, 3)));
    await tester.pumpAndSettle();
  ...
  testWidgets('hidden for packs that contain sub-packs', (tester) async {
    ...
    expect(find.text(l10n.setAsYourPath), findsNothing);
