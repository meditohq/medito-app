...
class User {
  ...
  User({required this.id, this.email, this.metadata});
...
class AuthRepositoryImpl extends AuthRepository {
  ...
  Future<void> initializeUser() async {
    try {
      ...
      var clientId = _preferences.getString(SharedPreferenceConstants.userId);
      dev.log(
        '[AUTH_REPO] Retrieved clientId from preferences: $clientId',
        level: 800,
      );
      ...
      dev.log(
        '[AUTH_REPO] isLoggedIn from SharedPreferences: $isLoggedIn',
        level: 800,
      );
      ...
      if (isLoggedIn) {
        ...
        if (refreshToken != null) {
          ...
          try {
            ...
            final backupToken = _preferences.getString(
              'medito_backup_refresh_token',
            );
        ...
        dev.log(
          '[AUTH_REPO] Set current user as anonymous: $_currentUser',
          level: 800,
        );
      }
    } catch (e, stackTrace) {
      ...
      try {
        ...
        while (!refreshSuccess && retryCount < maxRetries) {
          try {
            ...
            dev.log(
              '[AUTH_REPO] Refresh token invalid/expired',
              error: e,
              level: 800,
            );
            _crashlyticsService.recordError(
        ...
        if (_currentUser != null) {
          _currentUser = User(id: _tokens!.clientId, email: _tokens!.email);
          dev.log(
        ...
        dev.log(
          '[AUTH_REPO] Server error refreshing token',
          error: e,
          level: 800,
        );
        ...
        dev.log(
          '[AUTH_REPO] Refresh token error, clearing auth state',
          error: e,
          level: 800,
        );
  ...
  String? getUserEmail() {
    ...
    if ((email == null || email.isEmpty) && _tokens?.email != null) {
      dev.log(
        '[AUTH_REPO] Email empty but found in tokens: ${_tokens?.email}',
        level: 800,
      );
      if (_currentUser != null) {
        _currentUser = User(id: _currentUser!.id, email: _tokens!.email);
        email = _tokens!.email;
        dev.log(
          '[AUTH_REPO] Updated current user with token email: $email',
          level: 800,
        );
      }
    }
    ...
    if (email == null || email.isEmpty) {
      dev.log(
        '[AUTH_REPO] Email still empty, checking secure storage',
        level: 800,
      );
  ...
  Future<void> _retrieveEmailFromStorage() async {
    try {
      ...
      if (storedEmail != null && storedEmail.isNotEmpty) {
        ...
        if (_currentUser != null) {
          _currentUser = User(id: _currentUser!.id, email: storedEmail);
          dev.log(
            '[AUTH_REPO] Updated current user with stored email',
            level: 800,
          );
        }
      } else {
  ...
  Future<void> requestOtp(String email) async {
    try {
      var clientId =
          _preferences.getString(SharedPreferenceConstants.userId) ??
          _generateClientId();
  ...
  Future<bool> verifyOtp(String email, String otp) async {
    try {
      dev.log(
        '[AUTH_REPO] Starting OTP verification for email: $email',
        level: 500,
      );
      ...
      if (_tokens!.email != null) {
        ...
        dev.log(
          '[AUTH_REPO] Storing provided email parameter instead: $email',
          level: 500,
        );
      ...
      dev.log(
        '[AUTH_REPO] OTP verification completed successfully',
        level: 500,
      );
      return true;
    } catch (e) {
  ...
  Future<void> signInAnonymously() async {
    try {
      ...
      _tokens = await _authService.signIn(clientId: clientId);
  ...
  Future<void> _resetAuth() async {
    ...
    try {
      ...
      dev.log(
        '[AUTH_REPO] Error clearing Firebase Analytics user ID: $e',
        level: 800,
      );
    }
  ...
  Future<void> migrateEmailToStorage() async {
    ...
    if (!isLoggedIn) {
      dev.log(
        '[AUTH_REPO] User is not logged in, skipping email migration',
        level: 500,
      );
      return;
    }
    ...
    if (email == null || email.isEmpty) {
      dev.log(
        '[AUTH_REPO] No email in current state, checking /me endpoint',
        level: 500,
      );
      try {
        ...
        email = meData.email;
        dev.log(
          '[AUTH_REPO] Found email from /me endpoint: $email',
          level: 500,
        );
      } catch (e) {
        _crashlyticsService.recordError(
    ...
    if (email != null && email.isNotEmpty) {
      ...
      if (storedEmail == null) {
        dev.log(
          '[AUTH_REPO] Migrating email to secure storage: $email',
          level: 500,
        );
        await _secureStorage.storeUserEmail(email);
        ...
        if (_currentUser != null &&
          ...
          _currentUser = User(id: _currentUser!.id, email: email);
          dev.log(
            '[AUTH_REPO] Updated current user with migrated email',
            level: 500,
          );
        }
      }
      ...
      dev.log(
        '[AUTH_REPO] No email found to migrate, user may be anonymous',
        level: 500,
      );
    }
  }
...
final authRepositoryProvider = FutureProvider<AuthRepository>((ref) async {
  ...
  return AuthRepositoryImpl(preferences: preferences);
});
