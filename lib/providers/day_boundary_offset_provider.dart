...
/// into [StatsManager] so streak math honours it on every recalculation.
class DayBoundaryOffsetNotifier extends AsyncNotifier<int> {
  ...
  Future<int> build() async {
    ...
    StatsManager().setDayBoundaryOffset(Duration(hours: hours));
    return hours;
  }

  Future<void> setOffsetHours(int hours) async {
    await StatsManager().setDayBoundaryOffset(Duration(hours: hours));
